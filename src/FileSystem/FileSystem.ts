...
function DefaultFileSave(obj: any, key: string, file: ScreepMetaFile) {
  return (data_key: string, val: Json) => {
    const FILE_DATA = file.ToJson()
    if (FILE_DATA.can_delete) {
        obj[key] = undefined
        return
    }

    obj[key].tick_last_accessed = FILE_DATA.tick_last_accessed
    obj[key].path = FILE_DATA.path
    obj[key].file_name = FILE_DATA.file_name
...
export class FileSystem {
  ...
  private static ticks_passed: number = 0;
  ...
  public GetFile(path: string[]): ScreepFile {
    ...
    FILE.UpdateAccessFunctions(DefaultFileSave(FOLDER_OBJ, FILE_NAME, FILE), DefaultFileRead(FOLDER_OBJ, FILE_NAME));
