...
class StructureSupplyState extends CreepState implements EntityState {
  ...
  RunState() {
    ...
    const STRUCT_TYPES = <const>[STRUCTURE_SPAWN, STRUCTURE_EXTENSION];
    const SUPPLY_STRUCT_IDS = RoomData.GetRoomData()
      .GetOwnedStructureIds(...STRUCT_TYPES)
    ...
    for (let struct_type of STRUCT_TYPES) {
      target = STRUCT_MAP.get(struct_type)?.filter(s => s.store.getFreeCapacity(RESOURCE_ENERGY) > 0)
        .at(0);
      if (target != null) {
        break;
      }
...
export class HarvesterBehavior implements EntityBehavior {
  ...
  public RecieveOrder(order_data: JsonObj) {
    return () => ({});
  }
}
